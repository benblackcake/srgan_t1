...
def process_individual_image(filename_queue, img_size, random_crop=False):
    ...
    if random_crop:
        ...
        image = tf.image.pad_to_bounding_box(image, 0, 0, tf.maximum(img_size, image_shape[0]),
                                             tf.maximum(img_size, image_shape[1]))
...
def build_input_pipeline(filenames, batch_size, img_size, random_crop=False, shuffle=True, num_threads=1):
    print("call_build_input_pipeline")
...
def downsample_batch(batch, factor):
    print("__down_sample_shape__: %s "%batch.shape)
...
def evaluate_model(loss_function, get_batch, sess, num_images, batch_size):
    ...
    for i in range(int(math.ceil(num_images / batch_size))):
        ...
        loss += sess.run(loss_function,
                         feed_dict={'g_training:0': False, 'd_training:0': False, 'input_lowres:0': batch_lr,
                                    'input_highres:0': batch_hr})
        total += 1
    loss = loss / total
    return loss
