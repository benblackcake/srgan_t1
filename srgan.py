...
class SRGanGenerator:
    ...
    def __init__(self, discriminator, training, content_loss='mse', use_gan=True, learning_rate=1e-4, num_blocks=16,
                 num_upsamples=2):
    ...
    def _content_loss(self, y, y_pred):
        ...
        if self.content_loss == 'vgg22':
            ...
            return 0.006 * tf.reduce_mean(tf.square(vgg_y - vgg_y_pred)) + 2e-8 * tf.reduce_sum(
                tf.image.total_variation(y_pred))
    ...
    def _adversarial_loss(self, y_pred):
        ...
        return tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=y_discrim_logits, labels=tf.ones_like(y_discrim_logits)))
